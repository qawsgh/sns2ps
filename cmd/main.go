...
func main() {
	...
	match := entities.Match(matchURL, username, password, useLocal)
	fmt.Printf("Generating competitor list for \"%v\"\n", match.MatchName)

	squads := entities.Squads(squadsURL, username, password, useLocal)
	fmt.Printf("Found %d squads\n", len(squads))

	competitors := entities.Competitors(competitorsURL, categories, divisions, match, regions, squads, username, password, useLocal)
